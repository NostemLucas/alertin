...
Coordinates fetching from NIST, enrichment with CISA KEV, and database storage.
"""
...
class CVEProcessor:
    ...
    async def process_specific_cve(self, cve_id: str) -> Optional[CVE]:
        """
        Process a specific CVE by ID.
    ...
    async def process_cve_list(self, nist_vulnerabilities: list) -> dict:
        """
        Process a list of NIST vulnerabilities.
        ...
        with CVERepository() as repo:
            for nist_vuln in nist_vulnerabilities:
                try:
                    ...
                    existing = repo.get_by_id(cve.cve_id)
                    ...
                    if stats["cves_processed"] % 10 == 0:
                        logger.info(f"Processed {stats['cves_processed']}/{stats['cves_fetched']} CVEs")
